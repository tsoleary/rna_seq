...
```{r}
gwas_dgrp_reg_annot(gwas_hot_5) %>%
  dplyr::select(ID, gene, feature, pos, AvgMixedPval, contains("reg_")) %>%
  dplyr::filter(!is.na(reg_flybaseID)) %>%
  dplyr::rename("Distance to gene" = pos) %>%
  arrange(AvgMixedPval) %>%
  kable() %>%
  kable_styling("striped") %>%
...
```{r}
gwas_dgrp_reg_annot(gwas_cold_5) %>%
  dplyr::select(ID, gene, feature, pos, AvgMixedPval, contains("reg_")) %>%
  dplyr::filter(!is.na(reg_flybaseID)) %>%
  dplyr::rename("Distance to gene" = pos) %>%
  arrange(AvgMixedPval) %>%
...
### CTmin and CTmax Gene overlap

```{r}
gwas_hot %>%
  filter(gene %in% gwas_cold$gene & gene != "") %>%
  left_join(gwas_cold, by = ("gene"), suffix = c(".max", ".min")) %>%
  select(ID.max, ID.min, gene) %>%
  pivot_longer(cols = contains("ID"), names_to = "ID", values_to = "SNPs") %>%
  mutate(ID = case_when(ID == "ID.min" ~ "CTmin",
                        ID == "ID.max" ~ "CTmax")) %>%
  group_by(ID) %>%
...
## Differentially expressed GWAS hits
