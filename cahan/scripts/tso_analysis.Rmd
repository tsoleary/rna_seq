...
gwas_cold_all <- gwas_dgrp_annot(gwas_cold_all)
gwas_hot_all <- gwas_dgrp_annot(gwas_hot_all)
gwas_tb_all <- gwas_dgrp_annot(gwas_tb_all)


gwas_cold_5 <- gwas_dgrp_annot(gwas_cold_5)
gwas_hot_5 <- gwas_dgrp_annot(gwas_hot_5)
gwas_tb_5 <- gwas_dgrp_annot(gwas_tb_5)
...
gwas_cold_5 <- gwas_cold_5 %>%
  filter(AvgMixedPval < pval_cut)
gwas_hot_5 <- gwas_hot_5 %>%
  filter(AvgMixedPval < pval_cut)
gwas_tb_5 <- gwas_tb_5 %>%
  filter(AvgMixedPval < pval_cut)
...
## GWAS scatter & density plots  (10<sup>-5</sup>)
...
ggplot(deg_simple_distinct) +
  geom_line(mapping = aes(x = log2FoldChange.hot, color = gwas_g), 
            data = deg_simple_distinct %>%
              dplyr::filter(gwas_g != "CTmin"), stat = "density") +
  geom_line(mapping = aes(x = log2FoldChange.cold, color = gwas_g), 
            linetype = "dashed", data = deg_simple_distinct %>%
...
```{r density_plots_5_dia, fig.height = 4, fig.width = 7}
ggplot(deg_simple_distinct_4) +
  geom_line(mapping = aes(x = log2FoldChange.hot, color = gwas_g), 
...
## GWAS scatter with relevant density plot
...
```{r table_top_gwas_hot_feature_4}
gwas_hot_all %>%
  group_by(feature) %>%
  dplyr::count() %>%
  filter(feature != "") %>%
...
```{r table_top_gwas_cold_feature_4}
gwas_cold_all %>%
  group_by(feature) %>%
  filter(feature != "") %>%
  dplyr::count() %>%
...
```{r, include = FALSE}
ctmin_tf <- gwas_cold_all %>%
  dplyr::filter(AvgMixedPval < 10^-4) %>%
  dplyr::filter(gene %in% dmel_tf$SYMBOL) %>%
  dplyr::select(gene)

ctmax_tf <- gwas_hot_all %>%
  dplyr::filter(AvgMixedPval < 10^-4) %>%
  dplyr::filter(gene %in% dmel_tf$SYMBOL) %>%
  dplyr::select(gene)
