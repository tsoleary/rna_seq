...
# integrate the multiple genes back to the gwas df -----
...
for (i in 1:length(gene_list_hot)){
  if (length(gene_list_hot[[i]]) == 0){
    ...
    genes_hot[i] <- paste(gene_list_hot[[i]], collapse = " ")
  }
}
gwas_hot$gene <- genes_hot
...
# gwas_cold 
genes_cold <- c() 
for (i in 1:length(gene_list_cold)){
  if (length(gene_list_cold[[i]]) == 0){
    genes_cold[i] <- ""
  } else {
    genes_cold[i] <- paste(gene_list_cold[[i]], collapse = " ")
  }
}
gwas_cold$gene <- genes_cold
...
# get the grp info and combine back to comb df
