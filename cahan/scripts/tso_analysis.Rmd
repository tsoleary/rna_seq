...
# # old way to get gene annotations ----
...
# get all the gene symbols annotated for hot -----
hot_mult_genes <- str_split(gwas_hot$GeneAnnotation, ";")
annot_split_hot_mult <- sapply(hot_mult_genes, "str_split", "\\|")

gene_list_hot <- vector(mode = "list", length = length(annot_split_hot_mult))
...
for (i in 1:length(annot_split_hot_mult)) {
  ...
  gene_list_hot[[i]] <- unique_symbols
  
}


# get all the gene symbols annotated for cold -----

cold_mult_genes <- str_split(gwas_cold$GeneAnnotation, ";")
annot_split_cold_mult <- sapply(cold_mult_genes, "str_split", "\\|")

gene_list_cold <- vector(mode = "list", length = length(annot_split_hot_mult))

for (i in 1:length(annot_split_cold_mult)) {
  
  list_4  <- which(lengths(annot_split_cold_mult[[i]]) == 4)
  list_10 <- which(lengths(annot_split_cold_mult[[i]]) == 10)
  list_13 <- which(lengths(annot_split_cold_mult[[i]]) == 13)
  ...
  if(!is_empty(list_4)){
      for (k in list_4){
        x <- annot_split_cold_mult[[i]][[c(k, 2)]]
        sym_4 <- c(sym_4, x)
      }
  }
  ...
  if(!is_empty(list_10)){
      for (k in list_10){
        x <- annot_split_cold_mult[[i]][[c(k, 6)]]
        sym_10 <- c(sym_10, x)
      }
  }
  ...
  gene_list_cold[[i]] <- unique_symbols
  
}



# figure out how to intgrate it back in to have multiple rows for the unique

# sym_empty <- which(lengths(gene_list_hot) == 0)
# sym_one <- which(lengths(gene_list_hot) == 1)
# sym_mult <- which(lengths(gene_list_hot) >= 2)

genes <- c()

for (i in 1:length(gene_list_hot)){
  
  if (length(gene_list_hot[[i]]) == 0){
    genes[i] <- ""
  } else {
    genes[i] <- paste(gene_list_hot[[i]], collapse = " ")
  }
...
# need to add the $gene column before continuing.......
##########
