...
\begin{figure}[H]
  \includegraphics[width=\linewidth]{/Users/tsoleary/R/rna_seq/cahan/results/GO_results_files/go_fig_lfc_order.png}
\end{figure}
...
read_tsv("CTmin_ORA_030120_enrichment_results.txt") %>%
  select(description, FDR, userId) %>%
  mutate(FDR = signif(FDR, 3)) %>%
  rename(`GO category` = description, Genes = userId) %>%
...
read_tsv("CTmax_ORA_030120_enrichment_results.txt") %>%
  select(description, FDR, userId) %>%
  mutate(FDR = signif(FDR, 3)) %>%
  rename(`GO category` = description, 
         Genes = userId) %>%
  mutate(Genes = str_replace_all(Genes, ";", ", ")) %>%
  kable("latex", 
        booktabs = TRUE,
...
\href{https://tsoleary.github.io/rna_seq/cahan/results/GO_results_files/Cold_DEG_ORA_030120_enrichment_results.txt}{Cold DEG ORA results}

\href{https://tsoleary.github.io/rna_seq/cahan/results/GO_results_files/Hot_DEG_ORA_030120_enrichment_results.txt}{Hot DEG ORA results}
