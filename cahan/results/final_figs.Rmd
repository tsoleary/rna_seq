...
**Figure 4: Expression patterns of GWAS genes within significantly over-represented GO biological process categories.**  All expression patterns are expressed relative to the 25°C control. Bolded categories indicate those also over-represented in the full DEG dataset for the corresponding temperature extreme. Bolded genes indicate significant differential gene expression (FDR < 0.01).


# **Figure 5:** Effect size & MAF
...
p1 <- ggplot(gwas_hot_5 %>%
        ...
        labs(x = expression("Tolerance effect size" / sigma[p]), 
...
cowplot::plot_grid(p2, p1, nrow = 2, labels = "AUTO")
```

**Figure 5:** Scatter plot of the minor allele frequency versus the tolerance effect size normalized by the phenotypic variance ($\sigma_{p}$) for **(A)** CT<sub>min</sub> and  **(B)** CT<sub>max</sub>. 
