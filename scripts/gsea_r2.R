# I hope this works better than the last one ---------
...
GSEA = function(gene_list, GO_file, pval) {
  ...
  Up = fgsea::collapsePathways(ups, pathways = myGO, stats = gene_list,  
                               nperm = 500, pval.threshold = 0.05)
  Down = fgsea::collapsePathways(downs, myGO, gene_list,  nperm = 500, 
                                 pval.threshold = 0.05) 
...
GO_file = here::here("cahan/Drosophila_melanogaster_GSEA_GO_sets_all_symbols_April_2015.gmt")

res = GSEA(geneListMFer, GO_file, pval = 0.05)
dim(res$Results)
